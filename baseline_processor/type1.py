...
def time(raw):
    ...
    t = raw.replace('/', ' ').replace(':', ' ').replace('.', ' ').split()  # Changes all delimiters to space.
    if len(t) == 7:  # If day of the week NOT given.
        i = list(reversed(t[0:3])) + t[3:6]
    elif len(t) == 8:  # If day of the week given. e.g. 'Wed'
        i = list(reversed(t[1:4])) + t[4:7]
    else:
        raise TypeError

    try:
        if us_time:
            i[1], i[2] = i[2], i[1]  # Switch day and month to dd/mm/yy.
        return datetime.datetime(*[int(j) for j in i])
    except ValueError:  # Catches American date format, but only if day of month is greater than 12.
        i[1], i[2] = i[2], i[1]
        return datetime.datetime(*[int(j) for j in i])
...
for dirpath, dirnames, filenames in os.walk("."):
    for filename in [f for f in filenames if f.lower().endswith(".log")]:
        ...
        with open(file, 'r') as f:
            ...
            while True:
                ...
                try:
                    start_time = time(t)
                    tr += str(start_time) + '\n'
                    ip += str(start_time)  # Append datetime to tr and ip text.
                except TypeError:  # If can't process datetime or first line not datetime.
                    break
                ...
                while True:
                    ...
                    if line[0] == 'Pinging':
                        while True:
                            ...
                            x.append(run_time)
                            y.append(ping_time)
                        for _ in range(4):  # Skip four lines.
                            f.readline()
